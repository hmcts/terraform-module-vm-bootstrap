  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
      ...
      %{else}
      "fileUris": ${local.additional_template_file},
      "commandToExecute": "${var.additional_script_uri == null ? "" : "powershell -ExecutionPolicy Unrestricted -File ${var.additional_script_name} &&"} powershell -EncodedCommand ${local.template_file}"
      %{endif}
    }
    PROTECTED_SETTINGS
