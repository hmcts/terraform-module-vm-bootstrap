  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
      ...
      %{else}
      "fileUris": '${var.additional_script_name == null ? "" :["${var.additional_script_path}"]}',
      "commandToExecute": "'${var.additional_script_name == null ? "" : "powershell -ExecutionPolicy Unrestricted -File ${var.additional_script_name} |"}' powershell -EncodedCommand '${local.template_file}') | Out-File -filepath bootstrap_vm.ps1\" && powershell -ExecutionPolicy Unrestricted -File bootstrap_vm.ps1"
      %{endif}
    }
    PROTECTED_SETTINGS
