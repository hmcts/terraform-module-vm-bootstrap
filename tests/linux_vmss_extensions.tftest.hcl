...
run "setup_vm" {
  module {
    source = "./tests/modules/setup_vmss_linux"
  }
}
...
run "virtual_machine_scale_set_no_extensions" {
  ...
  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.azure_monitor[0].name == "AMALinux"
    error_message = "Incorrect name for azure monitor extension"
  }

  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.azure_monitor[0].type == "AzureMonitorLinuxAgent"
  ...
  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.custom_script[0].publisher == "Microsoft.Azure.Extensions"
    error_message = "Wrong publisher for a linux custom script"
  }

  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.custom_script[0].type == "CustomScript"
    error_message = "Wrong type for a linux custom script"
  }

  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.custom_script[0].type_handler_version == "2.1"
  ...
  assert {
    condition     = azurerm_virtual_machine_scale_set_extension.dynatrace_oneagent[0].type == "oneAgentLinux"
