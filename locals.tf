  ...
  template_file = base64encode(format("%s\n%s", templatefile("${path.module}/${local.bootstrap_vm_script}",
    {
      ...
      RHEL_CERT       = var.rhel_cert == null || var.rhel_cert == "" ? (length(data.azurerm_key_vault_certificate.rhel_cert) > 0 ? data.azurerm_key_vault_certificate.rhel_cert.certificate_data_base64 : "") : var.rhel_cert
